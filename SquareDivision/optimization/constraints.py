...
def linear_constraint(clinched_rectangles, east_neighbours, north_neighbours, idxs_to_close, keep_feasible=True):
    ...
    for idx_pair in idxs_to_close:
        arg_A, lb, ub = linear_args_closing_holes_brutal(idx_pair, clinched_rectangles)
        holes_A.append(arg_A)
        holes_lb.append(lb)
        holes_ub.append(ub)
    A  = np.vstack([low__X_A, low__Y_A, high_X_A, high_Y_A, cont_X_A, cont_Y_A] + holes_A)
    lb = np.hstack([low__X_rhs,low__Y_rhs,high_X_rhs,high_Y_rhs,cont_X_rhs,cont_Y_rhs,] + holes_lb)
    ub = np.hstack([low__X_rhs,low__Y_rhs,high_X_rhs,high_Y_rhs,cont_X_rhs,cont_Y_rhs,] + holes_ub)
