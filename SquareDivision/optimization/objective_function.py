...
def ratio_demand_cost(arg:np.ndarray, clinched_rectangles:np.ndarray=np.array([])):
    ...
    return cost, jac
#     # maybe multiplicative since with adding the comparabe size yelds wrong results 
#     arr = arg.reshape(-1,4).astype(float) # float beacuse np.reciprocal
#     origin_widths, recip__widths = clinched_rectangles[:, 2], np.reciprocal(arr[:,2])
#     originheights, recip_heights = clinched_rectangles[:, 3], np.reciprocal(arr[:,3])
#     ratio_d = np.zeros(shape=arr.shape)
#     cost = LA.norm(arr - clinched_rectangles)**2 + \
#         recip__widths.dot(origin_widths)/LA.norm(origin_widths,ord=1) + \
#         recip_heights.dot(originheights)/LA.norm(origin_widths,ord=1)
#     ratio_d[:,2] = (-1) * (recip__widths**2) * (origin_widths)/LA.norm(origin_widths,ord=1)
#     ratio_d[:,3] = (-1) * (recip_heights**2) * (originheights)/LA.norm(origin_widths,ord=1)
#     jac = (2*(arr - clinched_rectangles)).flatten() + ratio_d.flatten()
#     return cost, jac