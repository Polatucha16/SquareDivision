...
def down(v: np.ndarray,y: np.ndarray, h: np.ndarray):
    return v >= y + h / 2
def above_positive_slope(u: np.ndarray,v: np.ndarray,x: np.ndarray,y: np.ndarray,w: np.ndarray,h: np.ndarray):
    return v >=  (h/w) * (u - x - w/2) + y+h/2
def below_positive_slope(u: np.ndarray,v: np.ndarray,x: np.ndarray,y: np.ndarray,w: np.ndarray,h: np.ndarray):
    return v <=  (h/w) * (u - x - w/2) + y+h/2
def above_negative_slope(u: np.ndarray,v: np.ndarray,x: np.ndarray,y: np.ndarray,w: np.ndarray,h: np.ndarray):
    return v >= -(h/w) * (u - x - w/2) + y+h/2
def below_negative_slope(u: np.ndarray,v: np.ndarray,x: np.ndarray,y: np.ndarray,w: np.ndarray,h: np.ndarray):
    return v <= -(h/w) * (u - x - w/2) + y+h/2
...
def walls_in_scaled_view(rect:np.ndarray, dir:direction, suspended_walls:np.ndarray):
    ...
    scaled_wall_in_anchors_as = mid_pt[perp_idx] + np.abs(mid_pt[idx] - anchors) * (-1)* slope
    ...
    intervals_at_anchors = np.c_[suspended_walls[:,1:], scaled_wall_in_anchors_as, scaled_wall_in_anchors_bs]
    if len(suspended_walls) == 0:
        return np.array([]), np.array([]).reshape(0,2)
    have_intersection =  np.apply_along_axis(
...
def homogeneous_scale_in_dir_search(rect_num:int, rect_arr:np.ndarray, dir:direction, scale_or_push:mode):
    ...
    if scale_or_push == 'scale':
        (barrier_walls_anchors, 
         barrier_walls_intersections) = walls_in_scaled_view(rect, dir, all_walls)
    if scale_or_push == 'push':
        (barrier_walls_anchors, 
         barrier_walls_intersections) = walls_in_view(rect, dir, all_walls)
